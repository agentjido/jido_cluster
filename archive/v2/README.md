...
Use the Erlang/OTP 28 and Elixir 1.19 versions in `.tool-versions` with
asdf or mise. The locked Bedrock dependency fails to build its transaction
layout index on OTP 29 in the real Bedrock tests.

This package currently uses Jido V2 APIs. The sibling Jido V3 checkout is not
compatible with this package. Keep `JIDO_CLUSTER_USE_LOCAL_PATH_DEPS` unset
for this setup.
...
mix quality
mix test
mix test --include real_bedrock --include flaky --seed 0
```
